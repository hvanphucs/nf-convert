    ...
    def process(self, pipeline):
        ...
        for operation in operations:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Error processing operation {operation.name} {str(ex)}") from ex

        self.log_pipeline_info(
            pipeline.name, "pipeline processed", duration=(time.time() - t0_all))

        return LocalPipelineProcessorResponse()

    def export(self, pipeline, pipeline_export_format, pipeline_export_path, overwrite):
        raise NotImplementedError(
            "Local pipelines does not support export functionality")
...
class NotebookOperationProcessor(FileOperationProcessor):
    ...
    def process(self, operation: GenericOperation, elyra_run_name: str):
        ...
        # For local operations, papermill runs from this dir
    ...
    def process(self, operation: GenericOperation, elyra_run_name: str):
        ...
        try:
            ...
            if error_trim_index != -1:
                raise RuntimeError(
                    f"({file_name}): {error_msg[error_trim_index:].strip()}") from cpe
            else:
